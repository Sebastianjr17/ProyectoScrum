...
public class PromocionController {
	...
	// Método para verificar si el usuario es barman
	private boolean esBarman() {
		return "barman".equals(usuarioService.obtenerRolUsuarioActual());
	}

	// Mostrar la lista de promociones
	@GetMapping("/")
	public String listaPromociones(Model model) {
		model.addAttribute("esBarman", esBarman()); // Verificar si el usuario es barman
		...
		List<PromocionDTO> promocionesDTO = promociones.stream().map(promocion -> {
			List<String> nombreBebidas = promocion.getBebidasIds().stream()
					.map(bebidaId -> bebidas.stream().filter(bebida -> bebida.getId().equals(bebidaId))
							.map(Bebida::getNombre).findFirst().orElse("Desconocido"))
	...
	public String nuevaPromocion(Model model) {
		if (!esBarman()) {
			return "redirect:/promociones/"; // Redirige si no es barman
		}
	...
	public String guardarPromocion(@ModelAttribute("promocion") Promocion promocion,
		...
		// Si no se especifica el valor de "activo", lo establecemos en false por
		// defecto
